...
namespace WebWinMVC.Controllers
{
    ...
    public class AutomaticDataChangeController : ControllerBase
    {
        ...
        public async Task<IActionResult> FilterAndPivot([FromForm] string ApprovalDate, [FromForm] string FilterDay, [FromForm] int MFmonth = 0, [FromForm] int MonthOffset = 0)
        {
            try
            {
                ...
                foreach (var step in misSteps) 
                {
                    ...
                    IQueryable<DailyServiceReviewFormQuery> stepDataQuery = null; // 提前定义变量
                    ...
                    if (step == 3)
                    {
                        await ProcessAndStoreResultsAsyncToQuery(resultToStoreEditAndQuery,true);
                        await ProcessAndStoreResultsV91TempAsync(resultsToStore,true);
                    }
                    else
                    {
        ...
        public async Task ProcessAndStoreResultsAsyncToQuery(IEnumerable<PivotResult> resultsToStoreQuery,bool FirstTime=false)
        {
        ...
        public async Task ProcessAndStoreResultsV91TempAsync(IEnumerable<PivotResult> resultsToStore,bool firstTime=false)
        {
